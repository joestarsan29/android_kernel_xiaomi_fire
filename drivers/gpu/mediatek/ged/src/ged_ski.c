 ...
 *
 * Ported from android_kernel_xiaomi_gale (branch mb-mglru) to add the
 * standard /sys/kernel/gpu interface. This is what generic kernel
 * managers (e.g. Franco Kernel Manager) poll to display GPU frequency,
 * usage and model on MTK/Mali platforms - without it the GPU card in
 * those apps stays blank even though the underlying data exists via
 * mtk_gpufreq/GED.
 *
 * Note: mt_gpufreq_get_gpu_temp() does not exist in this tree's gpufreq
 * driver, so gpu_tmu is sourced from tscpu_curr_gpu_temp (exported by
 * the thermal driver) instead.
 */
...
/* Exported by drivers/misc/mediatek/thermal/mt6768 - avoids pulling in
 * the full tscpu_settings.h include path just for one field.
 */
